...
namespace Wkx
{
    internal class WkbWriter
    {
        ...
        {
            WriteDouble(point.X);
            wkbWriter.Write(point.Y ?? -double.NaN); WriteDouble(point.Y);


            if (dimension == Dimension.Xyz || dimension == Dimension.Xyzm) if (dimension == Dimension.Xyz || dimension == Dimension.Xyzm)
                    wkbWriter.Write(point.Z ?? -double.NaN); WriteDouble(point.Z);


            if (dimension == Dimension.Xym || dimension == Dimension.Xyzm) if (dimension == Dimension.Xym || dimension == Dimension.Xyzm)
                    wkbWriter.Write(point.M ?? -double.NaN); WriteDouble(point.M);
        }
